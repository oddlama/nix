...
static void buildDependencyList(EvalState & state, EvalState::TrackingScopeId scopeId, Value & v)
{
    ...
    if (debugTracking) {
        std::cerr << "[TRACK] buildDependencyList: looking for scopeId=" << scopeId
                  << ", trackingScopes.size()=" << state.trackingScopes.size() << "\n";
        for (auto & scope : state.trackingScopes) {
            std::cerr << "[TRACK]   scope id=" << scope.id << ", deps.size()=" << scope.deps.size() << "\n";
        }
    }
    ...
    if (!scope) {
        if (debugTracking) {
            std::cerr << "[TRACK] buildDependencyList: scope not found!\n";
        }
    ...
    if (debugTracking) {
        std::cerr << "[TRACK] buildDependencyList: found scope with " << scope->deps.size() << " deps\n";
    }
